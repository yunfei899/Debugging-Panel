...
8. IAR 工程首次迁移和目标配置方法见 `docs/XDS2xx共享调试面板迁移说明.md`；该文件名沿用历史名称，内容已改为 IAR/J-Link 方案。
...
4. 当前工程的 `HC_SXL.Debug.driver.xcl` 实际指定 `--drv_interface=SWD`。用户描述为 JTAG 时，连接前必须核对并同步物理接口与配置，不能根据描述自动改写。
